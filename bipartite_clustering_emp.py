    ...
    while num_of_round < 50:
        ...
        for idx_k in range(0, k_size):
            if idx_k in doc_nearest_dict:
                center_mtx[idx_k] = dev_csr_mtx[doc_nearest_dict.get(idx_k), :].mean(axis=0)
            else:
                pick = random.sample(range(row_num), k_size / 10)
                center_mtx[idx_k] = dev_csr_mtx[pick, :].mean(axis=0)

        # for cluster_num in doc_nearest_dict:
        #     center_mtx[cluster_num] = dev_csr_mtx[doc_nearest_dict.get(cluster_num), :].mean(axis=0)
...
def k_means_words(dev_csr_mtx, k_size):
    ...
    while num_of_round < 50:
        ...
        # cos_sim_mtx_ii = spatial.distance.cdist(dev_csr_mtx.toarray(), center_mtx, 'cosine')
        ...
        #     center_mtx[cluster_num] = dev_csr_mtx[word_nearest_dict[cluster_num], :].mean(axis=0)

        # compare the current dict with the previous one
        # for cluster_num in word_nearest_dict:
        #     cur_dict_size.append(len(word_nearest_dict[cluster_num]))
        # if pre_dict_size != []:
        #     print len(set(cur_dict_size) & set(pre_dict_size))
        #     if len(set(cur_dict_size) & set(pre_dict_size)) > k_size * 0.95:
        #         num_of_round = 50
