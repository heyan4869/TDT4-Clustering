...
def k_means_docs(dev_csr_mtx, k_size):
    ...
    for idx in range(0, k_size):
        pick = random.sample(range(row_num), k_size / 5)
    ...
    while num_of_round < 50:
        ...
        for idx_k in range(0, len(center_mtx)):
            if idx_k in doc_nearest_dict:
                ...
                # center_mtx[idx_k - del_row] = dev_csr_mtx[doc_nearest_dict[idx_k], :].mean(axis=0)
                # doc_nearest_dict[idx_k - del_row] = doc_nearest_dict.pop(idx_k)
            else:
                pick = random.sample(range(row_num), k_size / 5)
                center_mtx[idx_k] = dev_csr_mtx[pick, :].mean(axis=0)
                # del_row += 1
                # center_mtx = np.delete(center_mtx, (idx_k - del_row), axis=0)
        # center_mtx = np.delete(center_mtx, (del_row), axis=0)
