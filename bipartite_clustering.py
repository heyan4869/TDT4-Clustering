...
def k_means_docs(dev_csr_mtx, k_size):
    ...
    for idx in range(0, k_size):
        pick = random.sample(range(row_num), k_size / 10)
        # center_mtx[idx] = np.divide(np.asarray(dev_csr_mtx[pick, :].sum(0))[0].astype('float'), k_size / 10)
        center_mtx[idx] = dev_csr_mtx[pick, :].mean(axis=0)
    ...
    while num_of_round < 10:
        ...
        # TODO: !!!
        cos_sim_mtx = 1 - cos_sim(dev_csr_mtx.toarray(), center_mtx)
        ...
        for idx_k in range(0, k_size):
            if idx_k in doc_nearest_dict:
                ...
                pick = random.sample(range(row_num), k_size / 10)
                center_mtx[idx_k] = np.divide(np.asarray(dev_csr_mtx[pick, :].sum(0))[0].astype('float'), k_size / 10)

        print '\n'
        # TODO: change the converge condition
        if cur_sum_of_cos_dis < min_sum_cos_dis and min_sum_cos_dis - cur_sum_of_cos_dis > 3:
    ...
    while num_of_round < 20:
        ...
        for idx_k in range(0, k_size):
            if idx_k in word_nearest_dict:
                cur_k_word = word_nearest_dict.get(idx_k)
                num_of_line_in_cluster = len(cur_k_word)
                # calculate the row wise sum and then take average
                temp_word_sum = np.asarray(dev_csr_mtx[cur_k_word, :].sum(0))[0].astype('float')
                center_mtx[idx_k] = np.divide(temp_word_sum, num_of_line_in_cluster).astype('float')
            else:
                pick = random.sample(range(row_num), k_size / 10)
                center_mtx[idx_k] = np.divide(np.asarray(dev_csr_mtx[pick, :].sum(0))[0].astype('float'), k_size / 10)

        # TODO: 500
        if cur_sum_of_cos_dis < min_sum_cos_dis and min_sum_cos_dis - cur_sum_of_cos_dis > 500:
            # if more similar, update and continue
            min_sum_cos_dis = cur_sum_of_cos_dis
            print '\n'
            print min_sum_cos_dis
...
def bipartite_clustering():
    ...
    while num_of_round < 20:
        ...
        # for idx_k_word in range(0, word_k_size):
        #     if idx_k_word in word_nearest_dict:
        #         cur_k_word = word_nearest_dict.get(idx_k_word)
        #         num_of_line_word = len(cur_k_word)
        #         temp_word_sum = np.asarray(dev_csr_mtx[:, cur_k_word].sum(1)).astype('float')
        #         # if dev_csr_mtx_p is a numpy array, need to reshape
        #         dev_csr_mtx_p[:, idx_k_word] = np.divide(temp_word_sum, num_of_line_word)
        ...
        # for idx_k_doc in range(0, doc_k_size):
        #     if idx_k_doc in doc_nearest_dict:
        #         cur_k_doc = doc_nearest_dict.get(idx_k_doc)
        #         num_of_line_doc = len(cur_k_doc)
        #         temp_doc_sum = np.asarray(dev_csr_mtx[cur_k_doc, :].sum(0))[0].astype('float')
        #         dev_csr_mtx_pp[idx_k_doc] = np.divide(temp_doc_sum, num_of_line_doc).astype('float')
        ...
        dev_csr_mtx_word = dev_csr_mtx_pp.transpose()
